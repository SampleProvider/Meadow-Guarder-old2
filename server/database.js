...
storeDatabase = function(){
	...
	for(var i in Player.list){
		client.query('UPDATE progress SET username=\'' + Player.list[i].username + '\', progress=\'' + JSON.stringify({items:Player.list[i].inventory.items,xp:Player.list[i].xp,level:Player.list[i].level,img:Player.list[i].img,quest:Player.list[i].quest,questStage:Player.list[i].questStage,advancements:Player.list[i].advancements,worldRegion:Player.list[i].worldRegion,playTime:Player.list[i].playTime,version:VERSION}) + '\' WHERE username=\'' + Player.list[i].username + '\';', (err, res) => {
			if(err){
				throw err;
			}
		});
